...
local cap_flags = {
    [0] = SAVE_FLAG_HAVE_WING_CAP,
    [1] = SAVE_FLAG_HAVE_METAL_CAP,
    [2] = SAVE_FLAG_HAVE_VANISH_CAP,
    [3] =
        SAVE_FLAG_HAVE_WING_CAP,
    [4] = SAVE_FLAG_HAVE_WING_CAP
}
...
local function exclamation_box_act_initialize(obj)
    ...
    if obj.oBehParams2ndByte < 5 then
        ...
        --- for regular game is 3, for sm64 land its 5.
        obj.oAnimState = 5
        obj.oAction = EXCLAMATION_BOX_ACT_IDLE
    end
end
...
id_bhvExclamationBox = hook_behavior(id_bhvExclamationBox, OBJ_LIST_SURFACE, true, bhv_custom_exclamation_box_init,
    bhv_custom_exclamation_box_loop, "bhvExclamationBox")
...
local function bhv_custom_spawned_star_loop(obj)
    ...
    obj_set_model_extended(obj,
        ((1 << (obj.oBehParams >> 24)) & save_file_get_star_flags(get_current_save_file_num() - 1, gNetworkPlayers[0].currCourseNum - 1) ~= 0) and
        E_MODEL_TRANSPARENT_STAR or E_MODEL_STAR)
end

id_bhvSpawnedStar = hook_behavior(id_bhvSpawnedStar, OBJ_LIST_GENACTOR, false, nil, bhv_custom_spawned_star_loop,
    "bhvSpawnedStar")
