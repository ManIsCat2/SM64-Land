...
function tanooki_loop(m)
    if m.playerIndex == 0 then
        if gPlayerSyncTable[0].modelId == E_MODEL_TANOOKI_MARIO and tanooki then
            if m.action == ACT_JUMP or m.action == ACT_DOUBLE_JUMP or m.action == ACT_TRIPLE_JUMP or m.action == ACT_LONG_JUMP or m.action == ACT_SIDE_FLIP or m.action == ACT_WALL_KICK_AIR then
